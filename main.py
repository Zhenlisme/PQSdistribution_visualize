...
main_path="/".join(abspath(getsourcefile(lambda:0)).split("/")[:-1])
...
##AnoBed
os.system('python3 %s -a %s -opf %s'%(main_path+"/bin/GeneAnotation.py",gffile,anobed))

##G4SCI
os.system('python3 %s -AS %s -e %d -grun %d -opf %s'%(main_path+"/bin/conservation.py",Alignfile,Args.elong,Args.grun,G4sci))

##G4_coord
os.system('python3 %s -a %s -scif %s -opf %s'%(main_path+"/bin/G4_coord.py",anobed,G4sci,G4_coord))

##draw plot
PQStype="".join(["G",str(Args.grun),"-PQS"])
os.system('Rscript %s %s %s %s %s'%(main_path+"/bin/circlplot_simplified.r",G4_coord,PQStype,Args.text,circlplot))
